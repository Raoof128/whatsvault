...
def _setup(tmp_path):
    conn = C.open_db(str(tmp_path / "c.db"), os.urandom(32)); M.migrate(conn, "control")
    did = D.enroll(conn, "iphone", signing_pub=_sec1(ec.generate_private_key(ec.SECP256R1())),
                   agreement_pub=_sec1(agree := ec.generate_private_key(ec.SECP256R1())))
    conn.execute("INSERT INTO drafts(id, conversation_id, account_id, phone_number_id, recipient_wa_id, "
                 "body_bytes, kind, nonce, state) VALUES('drf_1','cnv','acc','PN1','61999',?,'text',?, "
                 "'PENDING_APPROVAL')", (b"hello", os.urandom(32)))
    conn.commit()
...
def test_structural_check_rejects_unknown_device(tmp_path):
    conn, did, agree = _setup(tmp_path)
    env = json.dumps({"device_id": "dev_nope", "draft_id": "drf_1", "decision": "APPROVE",
                      "signature_hex": "11" * 64, "nonce_hex": "bb" * 32}).encode()
...
def test_bad_signature_length_rejected(tmp_path):
    conn, did, agree = _setup(tmp_path)
    env = json.dumps({"device_id": did, "draft_id": "drf_1", "decision": "APPROVE",
                      "signature_hex": "11" * 10, "nonce_hex": "bb" * 32}).encode()
    with pytest.raises(R.RelayRejected):
        R.accept_envelope(conn, env)
