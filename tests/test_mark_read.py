...
def _msg(v, mid, conv, wamid, direction):
    v.execute("INSERT INTO messages(id, account_id, conversation_id, direction, ts_lower_ms, "
              ...
              "VALUES(?, 'acc',?,?,1,2,'ms','text','x','cloud_api',0,?)", (mid, conv, direction, wamid))
...
def _setup(tmp_path, *, with_grant=True):
    ...
    if with_grant:
        f = dict(capability_id="cap_1", device_id=did, account_id="acc", conversation_id="cnv",
                 action="MARK_READ", created_at_ms=1, expires_at_ms=10**12, max_actions=5, nonce=os.urandom(32))
...
def test_wamid_from_other_conversation_rejected(tmp_path):
    v, c = _setup(tmp_path)
    r = sender.mark_read(v, c, FakeMeta(), conversation_id="cnv", wamid="wamid.OTHER", account_id="acc", now_ms=100)
...
def test_outbound_wamid_rejected(tmp_path):
    v, c = _setup(tmp_path)
    r = sender.mark_read(v, c, FakeMeta(), conversation_id="cnv", wamid="wamid.OUT", account_id="acc", now_ms=100)
    assert r["reason"] == "NOT_INBOUND"


def test_no_grant_authorization_missing(tmp_path):
    v, c = _setup(tmp_path, with_grant=False)
    r = sender.mark_read(v, c, FakeMeta(), conversation_id="cnv", wamid="wamid.IN", account_id="acc", now_ms=100)
