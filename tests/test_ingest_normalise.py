...
def test_inbound_window_eligible_and_seconds_to_ms():
    m = [a for a in NM.split_webhook(WEBHOOK) if a["kind"] == "message"][0]
...
def test_echo_not_window_eligible():
    m = {"from": "61999", "id": "wamid.E", "timestamp": "1700000000", "type": "text",
         "text": {"body": "yo"}, "_wv_echo": True}
    atom = NM.split_webhook({"entry": [{"changes": [{"value":
        {"metadata": {"phone_number_id": "PN1"}, "messages": [m]}}]}]})[0]
...
def test_status_row_no_message_fk():
    st = [a for a in NM.split_webhook(WEBHOOK) if a["kind"] == "status"][0]
    r = NM.to_rows(st)
...
def test_unknown_value_no_domain_row():
    atoms = NM.split_webhook({"entry": [{"changes": [{"value":
        {"metadata": {"phone_number_id": "PN1"}, "weird_field": {"x": 1}}}]}]})
...
def test_semantic_keys_family_tagged_and_distinct():
    atoms = NM.split_webhook(WEBHOOK)
    m = [a for a in atoms if a["kind"] == "message"][0]
