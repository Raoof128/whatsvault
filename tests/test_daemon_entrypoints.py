...
@pytest.mark.parametrize("mod,blocked_on", [
...
@pytest.mark.parametrize("mod", ["apps.ingest.consumer", "apps.scheduler.scheduler",
                                 "apps.dispatcher.dispatch"])
...
def test_validate_accepts_successfulexit_false_as_crash_restart(tmp_path):
    ...
    plistlib.dump({"Label": "x", "ProgramArguments": ["a"], "RunAtLoad": True,
                   ...
                   "StandardErrorPath": "/x/logs/x.log"}, open(p, "wb"))
    f = {x["check"]: x["ok"] for x in launchd.validate(str(p))}
...
def test_validate_still_rejects_missing_keepalive(tmp_path):
    import plistlib
    p = tmp_path / "y.plist"
    plistlib.dump({"Label": "y", "ProgramArguments": ["a"], "RunAtLoad": True,
                   "StandardOutPath": "/x/logs/y.log",
                   "StandardErrorPath": "/x/logs/y.log"}, open(p, "wb"))
    f = {x["check"]: x["ok"] for x in launchd.validate(str(p))}
...
def test_validate_rejects_successfulexit_true(tmp_path):
    ...
    import plistlib
    p = tmp_path / "z.plist"
    plistlib.dump({"Label": "z", "ProgramArguments": ["a"], "RunAtLoad": True,
                   "KeepAlive": {"SuccessfulExit": True},
                   "StandardOutPath": "/x/logs/z.log",
                   "StandardErrorPath": "/x/logs/z.log"}, open(p, "wb"))
    f = {x["check"]: x["ok"] for x in launchd.validate(str(p))}
    assert f["keepalive_true"] is False
