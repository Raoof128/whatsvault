...
def test_run_startup_is_conservative(tmp_path):
    ...
    c.execute("INSERT INTO scheduled_jobs(job_id, conversation_id, generation_mode, enabled) "
              "VALUES('job_1','cnv','static',1)")
    did = D.enroll(c, "iphone", signing_pub=_sec1(ec.generate_private_key(ec.SECP256R1())),
                   agreement_pub=_sec1(ec.generate_private_key(ec.SECP256R1())))
    c.execute("INSERT INTO approvals(approval_id, draft_id, device_id, decision, nonce) "
              "VALUES('apv_1','drf_1',?, 'APPROVE', ?)", (did, os.urandom(32)))
    c.commit()
