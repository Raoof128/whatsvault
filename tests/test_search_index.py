...
def test_index_and_search_both_tiers(tmp_path):
    ...
    _msg(conn, "msg_1", "می‌روم"); _msg(conn, "msg_2", "hello world")
    IDX.index_message(conn, "msg_1", "می‌روم"); IDX.index_message(conn, "msg_2", "hello world")
...
def test_reindex_only_stale(tmp_path):
    conn = _vault(tmp_path)
    _msg(conn, "msg_1", "salam"); IDX.index_message(conn, "msg_1", "salam")
    ...
    assert conn.execute("SELECT normaliser_version FROM search_documents WHERE message_id='msg_1'"
                        ).fetchone()[0] == N.NORMALISER_VERSION
...
def test_rebuild_all_from_messages(tmp_path):
    conn = _vault(tmp_path)
    _msg(conn, "msg_1", "hello"); _msg(conn, "msg_2", "world")
    IDX.index_message(conn, "msg_1", "hello"); IDX.index_message(conn, "msg_2", "world")
    conn.execute("DELETE FROM search_documents"); conn.commit()  # wipe derived
