...
def _index_one(conn, message_id: str, text_original) -> None:
    ...
    row = conn.execute(
        "SELECT rowid, text_search, text_compact FROM search_documents WHERE message_id=?",
        (message_id,)).fetchone()
    if row:
        rid, old_s, old_c = row[0], row[1], row[2]
        conn.execute("INSERT INTO fts_lexical(fts_lexical, rowid, text_search) VALUES('delete', ?, ?)",
                     (rid, old_s))
        conn.execute("INSERT INTO fts_compact(fts_compact, rowid, text_compact) VALUES('delete', ?, ?)",
                     (rid, old_c))
        conn.execute("UPDATE search_documents SET normaliser_version=?, text_search=?, text_compact=? "
                     "WHERE rowid=?", (N.NORMALISER_VERSION, ts, tc, rid))
    else:
        cur = conn.execute(
            "INSERT INTO search_documents(message_id, normaliser_version, text_search, text_compact) "
            "VALUES(?,?,?,?)", (message_id, N.NORMALISER_VERSION, ts, tc))
        rid = cur.lastrowid
...
def reindex_stale(conn) -> int:
    rows = conn.execute(
        "SELECT message_id FROM search_documents WHERE normaliser_version != ?",
        (N.NORMALISER_VERSION,)).fetchall()
    for r in rows:
        mid = r[0]
