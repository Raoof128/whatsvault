...
def run_startup(vault_conn, control_conn, now_ms) -> dict:
    ...
    pending = control_conn.execute(
        ...
        "(SELECT 1 FROM approval_nonces n WHERE n.nonce = a.nonce)").fetchone()[0]
    return {"submitting_recovered": rec["recovered"], "scheduler_jobs": jobs,
            "circuit_state": dlq.state(vault_conn), "pending_approvals": pending}
