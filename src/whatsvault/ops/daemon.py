...
def open_databases(service: str):
    ...
    try:
        ks = KeyringKeyStore()
        return (C.open_existing("vault", p.vault_db, ks),
                C.open_existing("control", p.control_db, ks), None)
        ...
        return None, None, blocked(service, "keys_not_provisioned",
                                   f"{exc} absent; run `whatsvault mcp-provision` "
                                   "(or the vault/control key setup)")
    except keystore.KeyStoreError as exc:
        return None, None, blocked(service, "keystore_unavailable",
...
def blocked(service: str, blocked_on: str, detail: str) -> dict:
    return structlog.event({"service": service, "status": "not_started",
