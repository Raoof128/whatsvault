...
def prepare(control_conn, *, conversation_id, account_id, phone_number_id, recipient_wa_id,
            text, kind="text", now_ms, window_open, expires_at_ms=None) -> dict:
    ctx = {"recipient_wa_id": recipient_wa_id, "kind": kind, "account_ok": True, "now_ms": now_ms,
           "expires_at_ms": now_ms + 1, "device_active": True, "rate_ok": True,
           "recipient_is_group": False, "window_open": window_open}
    ...
    control_conn.execute(
        ...
        (draft_id, conversation_id, account_id, phone_number_id, recipient_wa_id, body, bsha, kind,
         canonical.attachments_digest([]), os.urandom(32), now_ms,
