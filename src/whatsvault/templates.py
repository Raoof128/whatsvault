...
def params_digest(template_name, language, definition_version, params) -> bytes:
    ...
    for i, p in enumerate(params or []):
        ...
        for field in (str(i), p.get("component_type", "body"), p.get("param_type", "text"),
                      str(p.get("value", ""))):
...
def upsert_from_sync(control_conn, rows) -> int:
    for r in rows:
        control_conn.execute(
            ...
            (r["template_id"], r.get("meta_template_id"), r["name"], r["language"], r.get("category"),
             r["status"], r.get("definition_version", 1), json.dumps(r.get("schema")), r.get("synced_at")))
    control_conn.commit()
    return len(rows)


def prepare_template(control_conn, *, conversation_id, account_id, phone_number_id, template_id,
                     params, now_ms) -> dict:
    ...
    control_conn.execute(
        ...
        (draft_id, conversation_id, account_id, phone_number_id, template_id, digest,
         __import__("os").urandom(32), now_ms, now_ms + DEFAULT_TTL_MS))
    control_conn.commit()
