...
def _minute_epoch_ms(rec: dict, tz_name: str, dst_resolutions: dict) -> int:
    ...
    local = dt.datetime(rec["year"], rec["month"], rec["day"], rec["hour"], rec["minute"], 0,
                        tzinfo=ZoneInfo(tz_name), fold=fold)
...
def dry_run(text: str, date_format: str, tz_name: str, *, self_participant_label) -> dict:
    ...
    dst_cases = [{"source_ordinal": r["source_ordinal"], "dst_class": r["dst_class"]}
                 for r in msgs if r["dst_class"] in ("fold", "nonexistent")]
    boundaries = [{"source_start_offset": r["source_start_offset"],
                   "source_end_offset": r["source_end_offset"]}
                  for r in records if r["kind"] == "ambiguous_boundary"]
...
def import_batch(vault_conn, text, *, source_sha256, date_format, tz_name, conversation_id,
                 account_id, self_participant_label, dst_resolutions=None, source_artifact=None) -> dict:
    dst_resolutions = dst_resolutions or {}
    # --- refusal gates (before any write) ---
    if not self_participant_label:
        raise ImportRefused("MISSING_SELF", "self_participant_label is required (direction cannot be guessed)")
    if not date_format:
    ...
    if date_format not in suggest_families(text):
        raise ImportRefused("FORMAT_MISMATCH", f"{date_format} not among validating families {suggest_families(text)}")
    ...
    for r in msgs:
        if r["dst_class"] in ("fold", "nonexistent") and r["source_ordinal"] not in dst_resolutions:
            raise ImportRefused("DST_UNRESOLVED",
                                f"source_ordinal {r['source_ordinal']} is a {r['dst_class']} instant")
    ...
    try:
        vault_conn.execute(
            ...
            (bat_id, source_sha256, art_path, art_sha, date_format, tz_name, self_participant_label,
             PARSER_VERSION, len(msgs),
        ...
        for r in msgs:
            ...
            occ[bucket] = idx + 1
            import_fp = F.import_fingerprint(FINGERPRINT_VERSION, conversation_id, minute_ms,
                                             r["sender"], mtype, content_fp, idx)
            ...
            if existing:
                ...
                vault_conn.execute(
                    ...
                    (msg_id, account_id, conversation_id, direction, iv.ts_lower_ms,
                     iv.ts_upper_ms_exclusive, iv.ts_precision, tz_name, "explicit_import_setting",
                     mtype, body, import_fp),
                )
                added += 1
            vault_conn.execute(
                ...
                (bat_id, msg_id, participant_id[r["sender"]], r["source_ordinal"],
                 r.get("source_start_offset"), r.get("source_end_offset"), content_fp,
                 FINGERPRINT_VERSION, PARSER_VERSION),
            )
            obs_count += 1
